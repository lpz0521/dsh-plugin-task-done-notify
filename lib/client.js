window.__ModuleLoader__.load({
   ...
   */
  function workspaceTitleOf(items, sessionId) {
   ...
   */
  function notificationBody(workspaceTitle, displayTitle) {
  ...
  /**
   * Diff two session-list snapshots for pending-interaction (agent waiting for
   * the user) transitions against a mutable notified-key set.
   *
   * Keys are `${sessionId}:${status}`, so the same interaction fires once, a
   * status change ('question' -> 'approval') fires again, and resolution or
   * session removal clears the keys.
   *
   * @returns newly-pending sessions [{ id, displayTitle, status }].
   */
  function diffPending(prev, next, notified) {
    const newly = [];
    for (const id of Object.keys(next)) {
      const row = next[id];
      if (typeof row.id !== "string") continue;
      const status = row.pendingInteraction;
  ...
  /**
   * Subscribe to the sessions list snapshot and diff running transitions plus
   * pending-interaction (agent asking the user) transitions. Settings are
   * re-read from localStorage on every event, so changes made in the settings
   * card take effect immediately.
  ...
  function observe(ctx, notify) {
    ...
    return list.subscribe(() => {
      ...
      const pending = diffPending(prevById, nextById, pendingNotified);
      prevById = nextById;
      const settings = loadSettings(localStorage);
      const wsItems = ctx.workspaces && ctx.workspaces.list
      ...
      for (const item of completed) {
        ...
        const wsTitle = workspaceTitleOf(wsItems, item.id);
        const label = wsTitle ? `${wsTitle} · ${item.displayTitle}` : item.displayTitle;
        logEvent(label, decision.ok, decision.reason);
        if (!decision.ok) continue;
        notify("任务完成 ✓", notificationBody(wsTitle, item.displayTitle));
      }
      // 等待回复：耗时门槛不适用（耗时是完成通知的概念）
      const pendingSettings = { ...settings, minDurationSec: 0 };
      for (const item of pending) {
        const decision = notifyDecision(pendingSettings, {
          parentId: item.parentId,
          hasFocus,
          elapsedSec: 0,
        });
        const wsTitle = workspaceTitleOf(wsItems, item.id);
        const label = wsTitle ? `${wsTitle} · ${item.displayTitle}` : item.displayTitle;
        logEvent(label + "（需要回复）", decision.ok, decision.reason);
        if (!decision.ok) continue;
        notify("需要你回复", notificationBody(wsTitle, item.displayTitle));
      }
    });
  }
  ...
  // ---- plugin entry ----
  const inject = ["sessions", "slots", "workspaces"];
